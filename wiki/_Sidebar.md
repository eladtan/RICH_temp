**[Home](Home)**

---

**Getting Started**
- [Getting Started](Getting-Started)
- [Build System](Build-System)
- [Running Simulations](Running-Simulations)

**User Guide**
- [Simulation Setup](Simulation-Setup)
- [Output & Visualization](Output-and-Visualization)
- [Equations of State](Equations-of-State)
- [Radiation Transport](Radiation-Transport)
- [Gravity](Gravity)
- [AMR](AMR)
- [Boundary Conditions](Boundary-Conditions)

**Architecture**
- [Code Architecture](Code-Architecture)
- [Mesh & Tessellation](Mesh-and-Tessellation)
- [Hydrodynamics](Hydrodynamics)
- [MPI Parallelism](MPI-Parallelism)

**Regression Tests**
- [Overview](Regression-Tests-Overview)
- [Running Tests](Running-Regression-Tests)
- [Test Catalog](Regression-Test-Catalog)

**Examples**
- [Sod Shock Tube](Example-Sod-Shock-Tube)
- [Sedov Blast Wave](Example-Sedov-Blast-Wave)
- [Marshak Wave](Example-Marshak-Wave)
- [Lane-Emden Star](Example-Lane-Emden)

**Reference**
- [FAQ](FAQ)
- [Troubleshooting](Troubleshooting)
- [Contributing](Contributing)
