...
Dependencies are discovered via `LD_LIBRARY_PATH` or explicit directory variables. Set the appropriate environment variable or module before running `build_rich.sh`. See [Build System](Build-System) for details.
...
See [Simulation Setup](Simulation-Setup) for details.

## Physics

### What equations of state are available?

IdealGas, Tillotson, OndrejEOS (tabular), and MixedEOS (composite). See [Equations of State](Equations-of-State).

### How does the moving mesh work?

RICH constructs a Voronoi tessellation from mesh-generating points. These points can move with the fluid (Lagrangian), stay fixed (Eulerian), or use a hybrid approach (RoundCells). The mesh is rebuilt each time step. See [Mesh and Tessellation](Mesh-and-Tessellation).
...
See [Radiation Transport](Radiation-Transport).

### How do I add a custom physics module?

Implement the appropriate abstract interface (`SourceTerm3D`, `PointMotion3D`, `CellUpdater3D`, etc.) and pass your implementation to `HDSim3D`. See [Code Architecture](Code-Architecture) for the list of interfaces.
...
Yes. See [Regression Tests Overview](Regression-Tests-Overview) for step-by-step instructions on adding a new test.
...
See [Output and Visualization](Output-and-Visualization).
