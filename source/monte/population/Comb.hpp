...
template<typename T, typename Grid>
std::vector<MonteCarloParticle<T, Grid>> StratifiedCombPopulationControl<T, Grid>::activate(const std::vector<MonteCarloParticle<T, Grid>> &particles)
{
    using MCParticle = MonteCarloParticle<T, Grid>;
    ...
    for(const MCParticle &particle : particles)
    {
        if(this->grid.IsPointOutsideBox(particle.location))
        {
            UniversalError eo("Stratified Comb Population Control: input particle is outside the box");
            eo.addEntry("Particle", particle);
            eo.addEntry("Cell count", Ncells);
            throw eo;
        }
        if(!std::isfinite(particle.weight))
        {
            UniversalError eo("Stratified Comb Population Control: particle weight is not finite");
            eo.addEntry("Particle weight", particle.weight);
            eo.addEntry("Particle cell index", particle.cellIndex);
            throw eo;
        }
        if(particle.weight < 0.0)
        {
            UniversalError eo("Stratified Comb Population Control: particle weight is negative");
            eo.addEntry("Particle weight", particle.weight);
            eo.addEntry("Particle cell index", particle.cellIndex);
            throw eo;
        }
        ...
        if(particle.cellIndex >= Ncells)
        {
            UniversalError eo("Stratified Comb Population Control: input particle has invalid cell index");
            eo.addEntry("Particle cell index", particle.cellIndex);
            eo.addEntry("Cell count", Ncells);
            throw eo;
        }
    ...
    if(!std::isfinite(totalWeight))
    {
        UniversalError eo("Stratified Comb Population Control: total particle weight is not finite");
        eo.addEntry("Total weight", totalWeight);
        eo.addEntry("Local particle count", particles.size());
        throw eo;
    }
    ...
    for(auto &p : result)
    {
        if(this->grid.IsPointOutsideBox(p.location))
        {
            if(p.cellIndex >= Ncells)
            {
                UniversalError eo("Stratified Comb Population Control: outside particle has invalid cell index");
                eo.addEntry("Particle cell index", p.cellIndex);
                eo.addEntry("Cell count", Ncells);
                throw eo;
            }
