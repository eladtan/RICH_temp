...
public:
    ...
    bool HandleAll(double fullDt, MonteCarloStepData &stepData, const std::vector<T> &normals);
...
private:
    ...
    std::vector<std::pair<std::vector<Vector3D>, std::vector<Vector3D>>> cellsFacesData;
...
template<typename T, typename Grid>
void MonteCarloManager2<T, Grid>::RankHandler::DistributedMutex::Destroy()
{
...
template<typename T, typename Grid>
void MonteCarloManager2<T, Grid>::RankHandler::DistributedMutex::Sync(void)
{
    ...
    grid(grid), comm_world(comm)
{
    ...
    {
        this->cellsFacesData.push_back(std::pair<std::vector<Vector3D>, std::vector<Vector3D>>());
        auto &[normals, onFaces] = this->cellsFacesData.back();
...
template<typename T, typename Grid>
MonteCarloManager2<T, Grid>::~MonteCarloManager2()
{
    size_t index = 0;
    for(rank_t rank1 = 0; rank1 < this->size_world; rank1++)
    {
        for(rank_t rank2 = 0; rank2 <= rank1; rank2++)
        {
            ...
            MPI_Comm &ranks_comm = this->communicators[index];
            MPI_Comm_free(&ranks_comm);
            index++;
        }
    }
}
...
template<typename T, typename Grid>
void MonteCarloManager2<T, Grid>::RankHandler::Sync(void)
{
    if(this->size_internal == 1)
    {
        return;
...
template<typename T, typename Grid>
void MonteCarloManager2<T, Grid>::RankHandler::Destroy()
{
    if(this->size_internal > 1)
    {
...
template<typename T, typename Grid>
void MonteCarloManager2<T, Grid>::RankHandler::RemoveParticles(const std::vector<size_t> &indicesInToHandle, size_t num)
{
...
template<typename T, typename Grid>
void MonteCarloManager2<T, Grid>::RankHandler::TransferParticles(const std::vector<const MCParticle*> &particles)
{
    static int minus_one = -1;
    static int plus_one = 1;
    ...
    if(this->size_internal > 1)
    {
        ...
        if(availLength == 0)
        {
            ...
            MPI_Accumulate(&increment, 1, MPI_INT, this->other_rank, MPI_MODE_NOCHECK, 1, MPI_INT, MPI_SUM, this->av_length_win);
...
template<typename T, typename Grid>
void MonteCarloManager2<T, Grid>::MonteCarloManager2::TransferParticles(rank_t rankBuffer, const std::vector<size_t> &indicesInToHandle, const std::vector<rank_t> &transferRanks, size_t num)
{
    if(indicesInToHandle.empty())
    {
        // nothing to transfer
...
template<typename T, typename Grid>
bool MonteCarloManager2<T, Grid>::MonteCarloManager2::HandleAll(double fullDt, MonteCarloStepData &stepData, const std::vector<T> &normals)
{
    ...
    for(size_t index = 0; index < activeRanksNum; index++)
    {
        ...
        for(int i = 0; i < length; i++)
        {
            ...
            const auto &[normalsOfFaces, pointsOnFaces] = this->cellsFacesData[particle.cellIndex];
            ...
            if(fabs(dt - timeIntersect) < 1e-16)
            {
                ...
                */ 
                particle.location = (particle.location + particle.velocity * dt) * (1 - MONTECARLO_EPSILON) + MONTECARLO_EPSILON * grid.GetMeshPoint(nextCellIndex);
...
typename MonteCarloManager2<T, Grid>::MonteCarloStepData MonteCarloManager2<T, Grid>::MonteCarloManager2::step(dt_t fullDt)
{
    ...
    while(not done)
    {
        this->HandleAll(fullDt, stepData, normals);
        this->progress->Sync();
    }
    ...
    size_t leavingNumber = stepData.leaving.size();
    size_t leavingTotal;
