...
#endif  // RICH_MPI
{
   ...
   private:
    ...
    void recalculateFields() {
        typename T::coord_type width =
            std::max(this->ur[0] - this->ll[0], std::max(this->ur[1] - this->ll[1], this->ur[2] - this->ll[2]));
        this->width = width;
   ...
   public:
    ...
    inline BoundingBox() : ll(T()), ur(T()) { this->recalculateFields(); };
    ...
    inline typename T::coord_type getWidth() const { return this->width; }

    inline typename T::coord_type getWidthSquared() const { return this->widthSquared; }
    ...
    inline bool intersects(const BoundingBox<U> &other) const {
        return this->contains(other.getLL()) or this->contains(other.getUR()) or other.contains(this->ll) or
               other.contains(this->ur);
    }
