...
public:
  ...
  //! \param includes_temperature Flag if to compute the temperature as well or not
	StressCellUpdater(vector<double> G0_arr, vector<double> Y0_arr, vector<double> dens0_arr, bool SR = false,double G=0, bool const includes_temperature = false);
...
private:
	...
	const vector<double> G0_arr_;
	const vector<double> Y0_arr_;
