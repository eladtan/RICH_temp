...
namespace {
    class CellEdgesGetter : public LazyList<Edge> {
       public:
        CellEdgesGetter(const Tessellation& tess, int n) : tess_(tess), edge_indices_(tess.GetCellEdges(n)) {}

        size_t size(void) const { return edge_indices_.size(); }

        Edge operator[](size_t i) const { return tess_.GetEdge(edge_indices_[i]); }
...
namespace {
    vector<Extensive> init_extensives(const Tessellation& tess, const PhysicalGeometry& pg,
        ...
        for (size_t i = 0; i < Nloop; ++i) {
            ...
            const double volume = pg.calcVolume(serial_generate(CellEdgesGetter(tess, static_cast<int>(i))));
            ...
            for (size_t j = 0; j < N; ++j) res[i].tracers[j] = cell.tracers[j] * mass;
        }
        return res;
    }
}  // namespace
...
hdsim::hdsim(
...
#endif
    Tessellation& tess, const OuterBoundary& obc, const PhysicalGeometry& pg, const vector<ComputationalCell>& cells,
    const EquationOfState& eos, const PointMotion& point_motion, const EdgeVelocityCalculator& evc,
    const SourceTerm& source, const TimeStepFunction& tsf, const FluxCalculator& fc, const ExtensiveUpdater& eu,
    const CellUpdater& cu, const pair<vector<string>, vector<string> >& tracer_sticker_names
#ifdef RICH_MPI
    ,
    ...
    :
      ...
      eos_(eos),
      cells_(cells),
      extensives_(init_extensives(tess, pg, cells, eos)),
...
#endif
{
    ...
    for (size_t i = 0; i < N; ++i) {
        ...
        for (size_t j = 0; j < sindex.size(); ++j) cells_[i].stickers[j] = cells[i].stickers[sindex[j]];
    }
}
...
void hdsim::TimeAdvance(void) {
    ...
    const vector<Extensive> fluxes = fc_(tess_, edge_velocities, cells_, extensives_, cache_data_, eos_, time_, dt);

    //  update_extensives(fluxes,
    eu_(fluxes, pg_, tess_, dt, cache_data_, cells_, extensives_, time_);

    ExternalForceContribution(tess_, pg_, cache_data_, cells_, fluxes, point_velocities, source_, time_, dt,
                              extensives_);

#ifdef RICH_MPI
    if (proc_update_ != 0) proc_update_->Update(proctess_, tess_);
...
void hdsim::TimeAdvanceClip(void) {
    ...
    extensives_ = extensives_ + FluxFix2(*oldtess, *oldtess, tess_, point_velocities, dt, cells_, fluxes,
...
namespace {
    vector<Extensive> average_extensive(const vector<Extensive>& extensives_1, const vector<Extensive>& extensives_2) {
        assert(extensives_1.size() == extensives_2.size());
        vector<Extensive> res(extensives_1.size());
        for (size_t i = 0; i < extensives_1.size(); ++i) res[i] = 0.5 * (extensives_1[i] + extensives_2[i]);
        return res;
    }
}  // namespace
...
void hdsim::TimeAdvance2Heun(void) {
    ...
    const vector<Extensive> mid_fluxes = fc_(tess_, edge_velocities, cells_, extensives_, cache_data_, eos_, time_, dt);

    vector<Extensive> mid_extensives = extensives_;

    ExternalForceContribution(tess_, pg_, cache_data_, cells_, mid_fluxes, point_velocities, source_, time_, dt,
                              mid_extensives);
    ...
    const vector<Extensive> fluxes =
        fc_(tess_, edge_velocities, mid_cells, mid_extensives, cache_data_, eos_, time_, dt);

    ExternalForceContribution(tess_, pg_, cache_data_, mid_cells, fluxes, point_velocities, source_, time_ + dt, dt,
                              mid_extensives);
...
namespace {
    ...
    template <class T>
    class AverageCalculator : public LazyList<T> {
       public:
        AverageCalculator(const vector<T>& ll1, const vector<T>& ll2) : ll1_(ll1), ll2_(ll2) {
            assert(ll1.size() == ll2.size());
        }

        size_t getLength(void) const { return ll1_.size(); }

        T operator()(size_t i) const { return 0.5 * (ll1_[i] + ll2_[i]); }
...
const PhysicalGeometry& hdsim::getPhysicalGeometry(void) const { return pg_; }

Tessellation& hdsim::getTessellation(void) { return tess_; }

const Tessellation& hdsim::getTessellation(void) const { return tess_; }

// Diagnostics

double hdsim::getTime(void) const { return time_; }

int hdsim::getCycle(void) const { return cycle_; }

const vector<ComputationalCell>& hdsim::getAllCells(void) const { return cells_; }

vector<ComputationalCell>& hdsim::getAllCells(void) { return cells_; }

void hdsim::recalculatePrimitives(void) { cells_ = cu_(tess_, pg_, eos_, extensives_, cells_, cache_data_); }
...
void hdsim::recalculateExtensives(void) {
    for (size_t i = 0; i < extensives_.size(); ++i) {
        ...
        extensives_[i].energy =
            eos_.dp2e(cell.density, cell.pressure, cell.tracers, ComputationalCell::tracerNames) * mass +
            0.5 * mass * ScalarProd(cell.velocity, cell.velocity);
        extensives_[i].momentum = mass * cell.velocity;
        //		extensives_[i].tracers.resize(cell.tracers.size());
        size_t N = cell.tracers.size();
        for (size_t j = 0; j < N; ++j) extensives_[i].tracers[j] = cell.tracers[j] * mass;
    }
}

void hdsim::setCycle(int cycle) { cycle_ = cycle; }

void hdsim::setStartTime(double t_start) { time_ = t_start; }

const EquationOfState& hdsim::getEos(void) const { return eos_; }

const OuterBoundary& hdsim::getOuterBoundary(void) const { return obc_; }

const vector<Extensive>& hdsim::getAllExtensives(void) const { return extensives_; }

vector<Extensive>& hdsim::getAllExtensives(void) { return extensives_; }

double hdsim::getCellVolume(size_t index) const {
    return pg_.calcVolume(serial_generate(CellEdgesGetter(tess_, static_cast<int>(index))));
}

const CacheData& hdsim::getCacheData(void) const { return cache_data_; }

#ifdef RICH_MPI
const Tessellation& hdsim::GetProcTessellation(void) const { return proctess_; }
#endif  // RICH_MPI
