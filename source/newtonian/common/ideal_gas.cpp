...
IdealGas::IdealGas(double const AdiabaticIndex)
    : g_(AdiabaticIndex),
      f_(std::numeric_limits<double>::signaling_NaN()),
      beta_(std::numeric_limits<double>::signaling_NaN()),
      mu_(std::numeric_limits<double>::signaling_NaN()) {}
...
double IdealGas::getAdiabaticIndex(void) const { return g_; }
...
double IdealGas::dT2cv(double const d, double const T, tvector const& /*tracers*/,
                       vector<string> const& /*tracernames*/) const {
    return f_ * beta_ * std::pow(T, beta_ - 1) * std::pow(d, 1 - mu_);
}

double IdealGas::de2T(double const d, double const e, tvector const& /*tracers*/,
                      vector<string> const& /*tracernames*/) const {
    return std::pow(std::pow(d, mu_) * e / f_, 1.0 / beta_);
}

double IdealGas::dT2e(double const d, double const T, tvector const& /*tracers*/,
                      vector<string> const& /*tracernames*/) const {
    return f_ * std::pow(T, beta_) * std::pow(d, -mu_);
}

double IdealGas::dT2p(double const d, double const T, tvector const& /*tracers*/,
                      vector<string> const& /*tracernames*/) const {
    return (g_ - 1.0) * f_ * std::pow(T, beta_) * std::pow(d, 1.0 - mu_);
}
double IdealGas::dp2T(double const d, double const p, tvector const& /*tracers*/,
                      vector<string> const& /*tracernames*/) const {
    return std::pow(p / ((g_ - 1.0) * f_ * std::pow(d, 1.0 - mu_)), 1.0 / beta_);
}
