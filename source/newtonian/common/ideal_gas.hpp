...
class IdealGas : public EquationOfState {
   ...
   public:
    ...
    double dp2e(double d, double p, tvector const& tracers = tvector(),
                vector<string> const& tracernames = std::vector<std::string>()) const override;
    ...
    double dT2cv(double const d, double const T, tvector const& tracers = tvector(),
                 vector<string> const& tracernames = std::vector<std::string>()) const override;

    double de2T(double const d, double const e, tvector const& tracers = tvector(),
                vector<string> const& tracernames = std::vector<std::string>()) const override;

    double dT2e(double const d, double const T, tvector const& tracers = tvector(),
                vector<string> const& tracernames = std::vector<std::string>()) const override;

    double dT2p(double const d, double const T, tvector const& tracers = tvector(),
                vector<string> const& tracernames = std::vector<std::string>()) const;

    double dp2T(double const d, double const p, tvector const& tracers = tvector(),
                vector<string> const& tracernames = std::vector<std::string>()) const;
};
