...
void SameRectangleIOHandler::dump(HDF5Writer &writer, const std::string &group, const Kernelization3D::IndexingKernel3D &kernel) const
{
    const auto &rect = static_cast<const SameRectangle &>(kernel);
...
std::shared_ptr<Kernelization3D::IndexingKernel3D> SameRectangleIOHandler::load(const HDF5Reader &reader, const std::string &group) const
{
    ...
    auto kernel = std::make_shared<SameRectangle>(std::vector<Vector3D>());
