...
PowerLawOpacity::PowerLawOpacity(double sigmaA0, double sigmaS0, double sigmaA_rho, double sigmaA_T, double sigmaS_rho, double sigmaS_T)
...
double PowerLawOpacity::getPlanckOpacity(const ComputationalCell3D &cell) const
{
    return this->sigmaA0 * std::pow(cell.density, this->sigmaA_rho) * std::pow(cell.temperature, this->sigmaA_T);
}

double PowerLawOpacity::getScatteringOpacity(const ComputationalCell3D &cell) const
{
    return this->sigmaS0 * std::pow(cell.density, this->sigmaS_rho) * std::pow(cell.temperature, this->sigmaS_T);
}

Vector3D PowerLawOpacity::getRandomVelocity(const ComputationalCell3D &cell) const
{
...
Vector3D PowerLawOpacity::getNewScatterVelocity(const ComputationalCell3D &cell, const MCParticle &particle) const
{
    static std::uniform_real_distribution<double> dist(-1+EPSILON, 1 - EPSILON);
    double x = dist(this->rng);
    double y = dist(this->rng);
