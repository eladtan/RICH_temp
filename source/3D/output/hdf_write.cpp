...
#define HDF5_WRITE_BLOCK_TAG 343
...
namespace
{
  ...
  template <class T>
  vector<T> read_vector_from_hdf5(const Group &file, const string &caption, const DataType &datatype)
  {
  ...
  vector<double> read_double_vector_from_hdf5(const Group &file, string const &caption)
  {
    return read_vector_from_hdf5<double>(file,
                                         caption,
                                         PredType::NATIVE_DOUBLE);
  }

  vector<int> read_int_vector_from_hdf5(const Group &file, const string &caption)
  {
    return read_vector_from_hdf5<int>(file,
                                      caption,
                                      PredType::NATIVE_INT);
  }

  vector<size_t> read_sizet_vector_from_hdf5(const Group &file, const string &caption)
  {
    return read_vector_from_hdf5<size_t>(file,
                                         caption,
...
temp = ReadSnapshot3D(filename + int2str(i) + ".h5", false);
    */
    Snapshot3D temp(mpi_write ? ReadSnapshot3D(filename, true, i) : ReadSnapshot3D(filename + int2str(i) + ".h5", false));
    if(i == 0)
    {
      snap.time = temp.time;
