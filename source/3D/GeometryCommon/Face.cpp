...
Face::Face(Face const& other): vertices(other.vertices), neighbors(other.neighbors){}

double Face::GetArea(void) const
{
  const Vector3D& ref = vertices[0];
  return inner_product(vertices.begin() + 1, vertices.end() - 1, vertices.begin() + 2, 0,
...
Vector3D calc_centroid(const Face& face)
{
  ...
  return inner_product(face.vertices.begin() + 1, face.vertices.end() - 1, face.vertices.begin() + 2, Vector3D(0,0,0),
