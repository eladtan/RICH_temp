...
void UpdateBox(Tessellation3D& tess, HDSim3D& sim, double const min_velocity, double const volume_fraction,
    ...
    Vector3D recvmax(temprecv[1] + 5 * temprecv[0], temprecv[2] + 5 * temprecv[0], temprecv[3] + 5 * temprecv[0]),
        recvmin(-temprecv[4] - 5 * temprecv[0], -temprecv[5] - 5 * temprecv[0], -temprecv[6] - 5 * temprecv[0]);
    bool recalc = false;
    ...
    if (recalc) {
        ...
        if (rank == 0) {
            ...
            std::cout << "Old box ll = " << cur_min.x << "," << cur_min.y << "," << cur_min.z << " ur = " << cur_max.x
                      << "," << cur_max.y << "," << cur_max.z << std::endl;
            std::cout << "New box ll = " << recvmin.x << "," << recvmin.y << "," << recvmin.z << " ur = " << recvmax.x
                      << "," << recvmax.y << "," << recvmax.z << std::endl;
        ...
        if (rank == 0) {
            ...
            while (counter < Np) {
                ...
                if (point.x < cur_min.x || point.y < cur_min.y || point.z < cur_min.z || point.x > cur_max.x ||
                    point.y > cur_max.y || point.z > cur_max.z) {
