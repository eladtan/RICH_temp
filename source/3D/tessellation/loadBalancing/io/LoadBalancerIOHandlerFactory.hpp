...
namespace LoadBalancerIO
{
    ...
    inline void writeLoadBalancer(HDF5Writer &writer, const std::string &group, const LoadBalancer &lb)
    {
    ...
    inline std::shared_ptr<LoadBalancer> readLoadBalancer(const HDF5Reader &reader, const std::string &group)
    {
        std::string name;
