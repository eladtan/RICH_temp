...
{
public:
    void dump(HDF5Writer &writer, const std::string &group, const LoadBalancer &lb) const override;

    std::shared_ptr<LoadBalancer> load(const HDF5Reader &reader, const std::string &group) const override;
};
