...
void RectangularConvertorIOHandler::dump(HDF5Writer &writer, const std::string &group, const HilbertConvertor3D &convertor) const
{
    ConvertorState state{convertor.getLL(), convertor.getUR(), convertor.getOrder()};
    writer.WriteElement(group + "/state", state);
}

std::shared_ptr<HilbertConvertor3D> RectangularConvertorIOHandler::load(const HDF5Reader &reader, const std::string &group) const
{
    ConvertorState state;
    reader.ReadElement(group + "/state", state);
    return std::make_shared<HilbertRectangularConvertor3D>(state.ll, state.ur, state.order);
}
