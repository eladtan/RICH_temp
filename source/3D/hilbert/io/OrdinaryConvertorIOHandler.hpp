...
{
public:
    void dump(HDF5Writer &writer, const std::string &group, const HilbertConvertor3D &convertor) const override;

    std::shared_ptr<HilbertConvertor3D> load(const HDF5Reader &reader, const std::string &group) const override;
};
