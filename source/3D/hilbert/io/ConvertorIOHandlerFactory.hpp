...
namespace ConvertorIO
{
    ...
    inline void writeConvertor(HDF5Writer &writer, const std::string &group, const HilbertConvertor3D &convertor)
    {
    ...
    inline std::shared_ptr<HilbertConvertor3D> readConvertor(const HDF5Reader &reader, const std::string &group)
    {
        std::string name;
