...
namespace CG {
    ...
    void mat_times_vec(const mat &sub_A_values, const size_t_mat &sub_A_indices, const std::vector<double> &v,
        ...
        for (size_t i = 0; i < sub_num_rows; i++) {
            dot_prod = 0;  // rezero the dot_prod buffer. we need this buffer so we can make it private to the thread to
                           // avoid race conditions.
            for (size_t j = 0; j < sub_num_cols; j++) {
                if (sub_A_indices[i][j] == max_size_t) break;
    ...
    std::vector<double> vector_rescale(std::vector<double> const &a, std::vector<double> const &b) {
        ...
        if (a.size() != b.size()) throw UniversalError("Sizes do not match in vector_rescale");
    ...
    void vec_lin_combo(double a, const std::vector<double> &u, double b, const std::vector<double> &v,
                       std::vector<double> &result) {
        if (u.size() != v.size()) throw UniversalError("Unequal vector sizes in vec_lin_combo");
        size_t n = u.size();
        result.resize(n, 0);
        for (size_t j = 0; j < n; j++) result[j] = a * u[j];
        for (size_t j = 0; j < n; j++) result[j] += b * v[j];
    }

    double mpi_dot_product(const std::vector<double> &sub_u,
                           const std::vector<double> &sub_v)  // need to pass it the buffer where to keep the result
    {
        if (sub_u.size() != sub_v.size()) throw UniversalError("Unequal vector sizes in vec_lin_combo");
...
#else
        for (size_t i = 0; i < loop_number; i += 4) {
            sub_prod += sub_u[i] * sub_v[i] + sub_u[i + 1] * sub_v[i + 1] + sub_u[i + 2] * sub_v[i + 2] +
                        sub_u[i + 3] * sub_v[i + 3];
        }
#endif

        for (size_t i = loop_number; i < length; i++) sub_prod += sub_u[i] * sub_v[i];
#ifdef RICH_MPI
    ...
    double mpi_dot_product2(const std::vector<double> &sub_u,
                            const std::vector<double> &sub_v)  // need to pass it the buffer where to keep the result
    {
        if (sub_u.size() != sub_v.size()) throw UniversalError("Unequal vector sizes in vec_lin_combo");
        size_t length = sub_u.size();
    ...
    void finalize_conjugate_gradient(size_t slice, size_t max_loc0, size_t max_loc1, int rank, size_t i, double error,
                                     double max_data_0_val, double max_data_1_val, double max_data_2_val,
                                     int max_data_0_id, int max_data_1_id, std::vector<double> &sub_x,
                                     std::vector<double> &sub_x_solution, std::vector<ComputationalCell3D> const &cells,
                                     Tessellation3D const &tess, int &total_iters, CG::mat const &A,
                                     size_t_mat const &A_indeces, std::vector<double> const &b, size_t Nlocal,
                                     double lengthscale, std::vector<double> &sub_a_times_p,
                                     std::vector<double> &sub_r) {
        max_loc0 /= slice;
        max_loc1 /= slice;
        if (cells.size() > max_loc0 and cells.size() > max_loc1) {
            if (rank == 0)
                std::cout << "Converged at iter = " << i << " error " << error << " negative value " << max_data_2_val
                          << std::endl;
            if (rank == max_data_0_id)
                std::cout << "Max0 " << max_data_0_val << " cell ID " << cells[max_loc0].ID << " density "
                          << cells[max_loc0].density << " temperature " << cells[max_loc0].temperature << " Er "
                          << cells[max_loc0].Erad * cells[max_loc0].density << " X " << tess.GetMeshPoint(max_loc0)
                          << " sub_x " << sub_x[max_loc0] << std::endl;
            if (rank == max_data_1_id)
                std::cout << "Max1 " << max_data_1_val << " cell ID " << cells[max_loc1].ID << " density "
                          << cells[max_loc1].density << " temperature " << cells[max_loc1].temperature << " Er "
                          << cells[max_loc1].Erad * cells[max_loc1].density << " X " << tess.GetMeshPoint(max_loc1)
                          << " sub_x " << sub_x[max_loc1] << std::endl;
        }
        total_iters = i;
#ifdef RICH_MPI
...
#ifdef RICH_MPI
        MPI_Reduce(rank == 0 ? MPI_IN_PLACE : dx_vec.data(), dx_vec.data(), dx_vec.size(), MPI_DOUBLE, MPI_SUM, 0,
                   MPI_COMM_WORLD);
#endif
        if (rank == 0) {
            std::cout << "Energy change due to sub_x_solution " << dx_vec[slice] << " with sign " << dx_vec[slice + 1]
                      << " negative_x " << dx_vec.back() << std::endl;
            // for(size_t i = 0; i < slice; ++i) std::cout << "energy change group " << i << " = " << dx_vec[i] <<
            // std::endl;
        }
    }
    ...
    std::vector<double> conj_grad_solver(
        const double tolerance, int &total_iters, Tessellation3D const &tess,
        std::vector<ComputationalCell3D> const &cells, double const dt, MatrixBuilder const &matrix_builder,
        double const time, std::vector<double> &sub_x_solution)  // total_iters is to store # of iters in it
    {
        ...
        for (int i = 0; i < max_iter; i++) {
            ...
            for (size_t j = 0; j < Nlocal; ++j) {
                ...
                if (std::abs(sub_r[j]) >
                    max_data[1].val *
                        (std::abs(A[j][0] *
                                  (std::abs(sub_x[j]) + std::numeric_limits<double>::min() * 100 + maxA[0] * 1e-9)))) {
                    max_data[1].val = std::abs(sub_r[j]) /
                                      (std::abs(A[j][0] * (std::abs(sub_x[j]) +
                                                           std::numeric_limits<double>::min() * 100 + maxA[0] * 1e-9)));
                    max_loc1 = j;
                }
                if (std::abs(sub_x[j] - old_x[j]) >
                    max_data[0].val *
                        (std::abs(sub_x[j]) + std::numeric_limits<double>::min() * 100 + maxA[0] * 1e-9)) {
                    max_data[0].val = std::abs(sub_x[j] - old_x[j]) /
                                      (std::abs(sub_x[j]) + std::numeric_limits<double>::min() * 100 + maxA[0] * 1e-9);
                    max_loc0 = j;
                }
                if (sub_x[j] < 0) {
            ...
            // recall that we can't have a 'break' within an openmp parallel region, so end it here then all threads are
            // merged, and the convergence is checked Convergence test
            if (sub_r_sqrd <
                    delta_init * tolerance  // std::sqrt(sub_r_sqrd_convergence / Ntotal) < tolerance * maxA [1]
                && max_data[1].val < 1e-6 && max_data[0].val < 1e-6 &&
                (i > 250 || max_data[2].val < 0.5)) {  // norm is just sqrt(dot product so don't need to use a separate
                                                       // norm fnc) // vector norm needs to use a all reduce!
                if (rank == 0)
                    std::cout << "Converged at iter = " << i << " delta " << sub_r_sqrd << " negative value "
                              << max_data[2].val << std::endl;
                if (rank == max_data[0].mpi_id)
                    std::cout << "Max0 " << max_data[0].val << " cell ID " << cells[max_loc0].ID << " density "
                              << cells[max_loc0].density << " temperature " << cells[max_loc0].temperature << " Er "
                              << cells[max_loc0].Erad * cells[max_loc0].density << " X "
                              << tess.GetMeshPoint(max_loc0).x << " Y " << tess.GetMeshPoint(max_loc0).y << " Z "
                              << tess.GetMeshPoint(max_loc0).z << std::endl;
                if (rank == max_data[1].mpi_id)
                    std::cout << "Max1 " << max_data[1].val << " cell ID " << cells[max_loc1].ID << " density "
                              << cells[max_loc1].density << " temperature " << cells[max_loc1].temperature << " Er "
                              << cells[max_loc1].Erad * cells[max_loc1].density << " X "
                              << tess.GetMeshPoint(max_loc1).x << " Y " << tess.GetMeshPoint(max_loc1).y << " Z "
                              << tess.GetMeshPoint(max_loc1).z << std::endl;
                total_iters = i;
                good_end = true;
#ifdef RICH_MPI
                MPI_exchange_data(tess, sub_x, true);
        ...
        if (not good_end) {
            ...
            if (rank == max_data[0].mpi_id) {
                ...
                std::cout << "Max0 " << max_data[0].val << " cell ID " << cells[max_loc0].ID << " density "
                          << cells[max_loc0].density << " temperature " << cells[max_loc0].temperature << " Er "
                          << cells[max_loc0].Erad * cells[max_loc0].density << " X " << tess.GetMeshPoint(max_loc0).x
                          << " Y " << tess.GetMeshPoint(max_loc0).y << " Z " << tess.GetMeshPoint(max_loc0).z
                          << " volume " << tess.GetVolume(max_loc0) << " max volume " << max_volume << " min volume "
                          << min_volume << std::endl;
            }
            if (rank == max_data[1].mpi_id) {
                ...
                std::cout << "Max1 " << max_data[1].val << " cell ID " << cells[max_loc1].ID << " density "
                          << cells[max_loc1].density << " temperature " << cells[max_loc1].temperature << " Er "
                          << cells[max_loc1].Erad * cells[max_loc1].density << " X " << tess.GetMeshPoint(max_loc1).x
                          << " Y " << tess.GetMeshPoint(max_loc1).y << " Z " << tess.GetMeshPoint(max_loc1).z
                          << " volume " << tess.GetVolume(max_loc1) << " max volume " << max_volume << " min volume "
                          << min_volume << std::endl;
            }
    ...
    std::vector<double> BiCGSTAB(const double tolerance, int &total_iters, Tessellation3D const &tess,
                                 std::vector<ComputationalCell3D> const &cells, double const dt,
                                 MatrixBuilder const &matrix_builder, double const time,
                                 std::vector<double> &sub_x_solution,
                                 bool &good_end)  // total_iters is to store # of iters in it
    {
        size_t slice = 1;
        ...
        for (int i = 0; i < max_iter; i++) {
            ...
            if (std::abs(rho0) < std::numeric_limits<double>::min() * 1e100) {
                if (rank == 0) std::cout << "Exited BiCGSTAB: rho0 is very small: " << rho0 << std::endl;
                finalize_conjugate_gradient(slice, max_loc0, max_loc1, rank, i, error, max_data[0].val, max_data[1].val,
                                            max_data[2].val, max_data[0].mpi_id, max_data[1].mpi_id, sub_x,
                                            sub_x_solution, cells, tess, total_iters, A, A_indeces, b, Nlocal,
                                            matrix_builder.GetLengthScale(), sub_a_times_p, sub_r);
                good_end = true;
                break;
            }
            if (print) std::cout << "iter " << i << " rho0 " << rho0 << " sub_r_sqrd " << sub_r_sqrd << std::endl;
            // r_old = sub_r;                 // Store previous residual
            sub_r_sqrd_old = sub_r_sqrd;  // save a recalculation of r_old^2 later
            ...
            double const alpha =
                std::abs(sub_r0_v) < std::numeric_limits<double>::min() * 1e100 ? 0.0 : rho0 / sub_r0_v;
            if (print) std::cout << "alpha " << alpha << " sub_r0_v " << sub_r0_v << std::endl;
            ...
            if (print) std::cout << "w " << w << " up " << up << " down " << down << std::endl;
            if (std::abs(alpha) < std::numeric_limits<double>::min() * 1e100 &&
                std::abs(w) < std::numeric_limits<double>::min() * 1e100) {
                if (rank == 0)
                    std::cout << "Exited BiCGSTAB: alpha and w very small: alpha " << alpha << " w " << w << " rho0 "
                              << rho0 << std::endl;
                finalize_conjugate_gradient(slice, max_loc0, max_loc1, rank, i, error, max_data[0].val, max_data[1].val,
                                            max_data[2].val, max_data[0].mpi_id, max_data[1].mpi_id, sub_x,
                                            sub_x_solution, cells, tess, total_iters, A, A_indeces, b, Nlocal,
            ...
            if (print) std::cout << "iter " << i << " error " << error << std::endl;
            // Convergence test
            if (error < tolerance) {
                ...
                max_data[1].val = 0;
                max_data[2].val = 0;
                for (size_t j = 0; j < Nlocal; ++j) max_sub_x = std::max(max_sub_x, std::abs(sub_x[j]));
#ifdef RICH_MPI
                MPI_Allreduce(MPI_IN_PLACE, &max_sub_x, 1, MPI_DOUBLE, MPI_MAX, MPI_COMM_WORLD);
#endif
                for (size_t j = 0; j < Nlocal; ++j) {
                    ...
                    if (std::abs(sub_r[j]) >
                        max_data[1].val *
                            (std::abs(A[j][0] * (std::abs(sub_x[j]) + std::numeric_limits<double>::min() * 100 +
                                                 max_sub_x * 1e-5)))) {
                        max_data[1].val =
                            std::abs(sub_r[j]) /
                            (std::abs(A[j][0] * (std::abs(sub_x[j]) + std::numeric_limits<double>::min() * 100 +
                    ...
                    // if(std::abs(sub_x[j] - old_x[j]) > max_data[0].val * (std::abs(sub_x[j]) +
                    // std::numeric_limits<double>::min() * 100 + maxA[0] * 1e-9))
                    // {
                    //     max_data[0].val = std::abs(sub_x[j] - old_x[j]) / (std::abs(sub_x[j]) +
                    //     std::numeric_limits<double>::min() * 100 + maxA[0] * 1e-9); max_loc0 = j;
                    // }
                ...
                MPI_Allreduce(MPI_IN_PLACE, max_data, 3, MPI_DOUBLE_INT, MPI_MAXLOC, MPI_COMM_WORLD);
#endif
                if ((max_data[1].val < 1e-6 && max_data[0].val < 1e-6 &&
                     ((i > 25 && max_data[1].val < 1e-10) || max_data[2].val < 0.5)) ||
                    error < 1e-100) {  // norm is just sqrt(dot product so don't need to use a separate norm fnc) //
                                       // vector norm needs to use a all reduce!
                    finalize_conjugate_gradient(
                        slice, max_loc0, max_loc1, rank, i, error, max_data[0].val, max_data[1].val, max_data[2].val,
                        max_data[0].mpi_id, max_data[1].mpi_id, sub_x, sub_x_solution, cells, tess, total_iters, A,
                        A_indeces, b, Nlocal, matrix_builder.GetLengthScale(), sub_a_times_p, sub_r);
                    good_end = true;
                    break;
                }
            }
            ...
            if (print)
                std::cout << "beta " << beta << " rho0_new " << rho0_new << " rho0 " << rho0 << " w " << w << " alpha "
                          << alpha << std::endl;
        ...
        if (not good_end) {
            ...
            max_loc1 /= slice;
            if (rank == 0)
                std::cout << "not good end, delta " << sub_r_sqrd << " maxdata2 " << max_data[2].val << " iterations "
                          << total_iters << " scale_b " << scale_b << std::endl;
            if (rank == max_data[0].mpi_id)
                std::cout << "Max0 " << max_data[0].val << " cell ID " << cells[max_loc0].ID << " density "
                          << cells[max_loc0].density << " temperature " << cells[max_loc0].temperature << " Er "
                          << cells[max_loc0].Erad * cells[max_loc0].density << " X " << tess.GetMeshPoint(max_loc0).x
