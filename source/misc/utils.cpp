...
vector<double> linspace(double xl, double xh, int n) {
    ...
    for (size_t i = 0; i < size_t(n); ++i)
        res[i] = xl + (xh - xl) * static_cast<double>(i) / static_cast<double>(n - 1);
    return res;
}
...
vector<double> arange(double x_min, double x_max, double dx) {
    ...
    double x = x_min - dx;
    std::generate(res.begin(), res.end(), [&x, &dx]() mutable {
        x += dx;
        return x;
    });
    /*
...
double min(vector<double> const& v) {
    double res = v[0];
    for (size_t i = 1; i < v.size(); ++i) res = std::min(res, v[size_t(i)]);
    return res;
}

double max(vector<double> const& v) {
    double res = v[0];
    for (size_t i = 1; i < v.size(); ++i) res = std::max(res, v[size_t(i)]);
    return res;
}

double fastsqrt(double x) {
#ifdef USE_VCL_VECTORIZATION
    if (x < static_cast<double>(FLT_MIN) || x > static_cast<double>(FLT_MAX)) return std::sqrt(x);
...
double Interpolate2DTable(double const x, double const y, std::vector<double> const& x_vec,
                          std::vector<double> const& y_vec, std::vector<std::vector<double>> const& data,
    ...
    if (x_vec.size() <= slope_length || y_vec.size() <= slope_length)
        throw UniversalError("Interpolate2DTable: x_vec.size() <= slope_length || y__vec.size() <= slope_length");
    ...
    if (x < x_vec[0]) {
        if (y < y_vec[0]) {
            ...
            double const x_vec_slope =
                (BiLinearInterpolation(x_vec, y_vec, data, x_vec[slope_length - 1], y) - data_x0) /
                (x_vec[slope_length - 1] - x_vec[0]);

            // Perform interpolation using the calculated slope
            return std::exp(BiLinearInterpolation(x_vec, y_vec, data, x_vec[0] * 1.00001, y) +
                            x_vec_slope * (x - x_vec[0]));
        }
    }
    ...
    if (x > x_vec.back()) {
        if (y < y_vec[0]) {
            ...
            double const y_slope = (data[x_vec.size() - 1][slope_length - 1] - data[x_vec.size() - 1][0]) /
                                   (y_vec[slope_length - 1] - y_vec[0]);

            // Perform interpolation using the calculated slope and x_vec_high_slope
            return std::exp(data[x_vec.size() - 1][0] + y_slope * (y - y_vec[0]) +
                            x_vec_high_slope * (x - x_vec.back()));
        } else
            // Perform interpolation using the calculated slope and x_vec_high_slope
            return std::exp(BiLinearInterpolation(x_vec, y_vec, data, x_vec.back() * 0.99999, y) +
                            x_vec_high_slope * (x - x_vec.back()));
    }
    ...
    if (y < y_vec[0]) {
        ...
        // Calculate the slope for y
        double const y_slope = (BiLinearInterpolation(x_vec, y_vec, data, x, y_vec[slope_length - 1]) - data_y0) /
                               (y_vec[slope_length - 1] - y_vec[0]);
