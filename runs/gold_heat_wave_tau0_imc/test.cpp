// Subsonic ablative heat wave in gold — Case 1 (tau = 0), IMC radiation
// Same physics/setup as runs/gold_heat_wave_tau0/ but Monte Carlo (IMC) instead of diffusion.
...
#include "source/monte/boundary/MovingSideTemperature.hpp"
...
int main(int argc, char* argv[])
{
	...
	auto boundaryCond = std::make_shared<MovingSideTemperature<Vector3D, Tessellation3D>>(
		tess, T_bath_init, boundaryPhotonsPerCell);

	// diffusionPressureGradient=false: per-particle MC momentum (absorption + scatter recoil)
	...
	if (rank == 0)
		std::cout << "Gold heat wave tau=0 (IMC): Nx=" << Nx
