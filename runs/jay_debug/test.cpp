...
// #define test_run 1
// #define disable_full_gravity 1
// #define mem_profile 1

#ifdef mem_profile
#include "utils/debug/MemoryRecorder.hpp"
#endif
...
namespace
{
	...
	vector<ComputationalCell3D> GetCells(Tessellation3D const &tess, double M, double R, OndrejEOS const &eos, double const Punits, double const n)
	{
		...
		if(n > 2)
		{
			xsi = read_vector("/home/yhe/RICH/data/xsi3.txt");
			theta = read_vector("/home/yhe/RICH/data/theta3.txt");
			endfactor = 2.0182359;
		}
		else
		{
			xsi = read_vector("/home/yhe/RICH/data/xsi32.txt");
			theta = read_vector("/home/yhe/RICH/data/theta32.txt");
			endfactor = 2.714055;
		}
		xsi[0] = 0;
...
int main(void)
{
	...
	std::string run_directory("/scratch-shared/yhe/rich-data/");
...
// #endif
	...
	std::string eos_location("/home/yhe/RICH/data/EOS/");
	std::string STA_location("/home/yhe/RICH/data/STA/");
	bool const full_gravity =
#ifndef disable_full_gravity
	fs::exists(gravity_name);
#else
	false;
#endif
	if(full_gravity)
	...
	if(restart &&
	    ...
	    full_gravity &&
#endif
	    (not fs::exists(file_name + int2str(counter) + ".h5")))
	{
		file_name += "full_";
		if(rank == 0)
	...
	//Radiation
	STAgreyOpacity opacity("/home/yhe/RICH/data/STA/");
	if (rank == 0)
	...
	Diffusion matrix_builder(opacity, d_boundary, eos, std::vector<std::string>(), flux_limit, hydro_on, compton_on, cooling_time_limiter_on);
	...
	CourantFriedrichsLewy tsf(0.275, 1, force, std::vector<std::string> (),
#ifdef test_run
	true);
#else
	false);
	...
	double t_apo = 6 * std::sqrt(apocenter * apocenter * apocenter / Mbh);
	double tf = 2 * t_apo;  // Set tf to 2 times the apocenter time
	double mindt = 0.001;
	double nextT = 0;
	...
	double step_time = 0;
	double const total_wtime_limit = 18 * 3600; // 18 hours, set to 0 to disable
	double const restart_wtime = 15000;
	double const min_dt_output = 0.05 * std::sqrt(std::pow(R, 3.0) * Mbh / M);

#ifdef test_run
	int cycle_end = sim->getCycle() + 100;
	while (sim->getCycle() < cycle_end) // for benchmark
#else
	while (sim->getTime() < tf)
#endif
	{
#ifdef mem_profile
  		MemoryRecorder::Record("Cycle " + std::to_string(sim->getCycle()));
#endif
		if (sim->getCycle() % 1 == 0)
		{
		...
		try
		{
			...
			if (restart_dump == 1)
			{
				...
				if (MPI_Wtime() > total_wtime_limit && total_wtime_limit > 0)
				{
                    if(rank == 0)
                        std::cout<<"Total wall time limit reached, exiting"<<std::endl;
					MPI_Finalize();
					return 0;
				}
	#endif // test_run
...
#endif //RICH_MPI
#ifdef mem_profile
			MemoryRecorder::Record("Before radiation step");
#endif
			double new_dt = sim->RadiationTimeStep(old_dt, matrix_builder);
#ifdef mem_profile
			MemoryRecorder::Record("After radiation step");
#endif
...
#ifdef mem_profile
			MemoryRecorder::Record("Before hydro step");
#endif
			sim->timeAdvance2();
			if (rank == 0)
				std::cout << "Finished hydro step" << std::endl;
#ifdef mem_profile
			MemoryRecorder::Record("After hydro step");
#endif
#ifdef disable_full_gravity // still do AMR for benchmarking purposes
...
#ifdef mem_profile
					MemoryRecorder::Record("Before AMR");
#endif
					amr(*sim);
#ifdef mem_profile
					MemoryRecorder::Record("After AMR");
#endif
				}
#ifdef remove_center
			if(full_gravity)
...
#ifdef mem_profile
		MemoryRecorder::Record("End of cycle");
		MemoryRecorder::PrintRecords(std::cerr);
