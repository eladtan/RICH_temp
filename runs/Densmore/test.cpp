...
struct Case {
    ...
    double const sigma_0_nom_left;
    double const sigma_0_nom_right;
    double const system_length;
	double const t_final;
    std::optional<double> const step_coordinate;
};
...
Case get_case(std::string_view const case_num_sv){
    ...
    switch(case_num){
        case 0:
        	return {"10", 10.0*std::pow(kev, 3.5), 10.0*std::pow(kev, 3.5), 5.0, 1e-9, std::nullopt};
        case 1:
			return {"100", 100.0*std::pow(kev, 3.5), 100.0*std::pow(kev, 3.5), 3.0, 1e-9,  std::nullopt};
			
		case 2:
			return {"1000", 1000.0*std::pow(kev, 3.5), 1000.0*std::pow(kev, 3.5), 1.0, 1e-9, std::nullopt};
		
		case 3:
			return {"Step", 10.0*std::pow(kev, 3.5), 1000.0*std::pow(kev, 3.5), 3.0, 1e-9, 2.0};
			
		case 4:
			return {"Step2", 1000.0*std::pow(kev, 3.5), 10.0*std::pow(kev, 3.5), 1.5, 5e-9, 0.5};
...
int main(int argc, char *argv[])
{
	...
	std::cout << "Running Densmore case: " << current_case.description << ", With " << G << " Energy groups" << std::endl;
    ...
    //Radiation
    double const sigma_0_nom_left =  current_case.sigma_0_nom_left;
	double const sigma_0_nom_right = current_case.sigma_0_nom_right;
	...
	{	
		cells[i].tracers[0] = 0.0;
		cells[i].tracers[1] = 0.0;

		if(current_case.step_coordinate){
			if(tess.GetCellCM(i).x < current_case.step_coordinate.value())
	...
	double const tf = current_case.t_final / tscale;
