...
# The analytical solver convention:  small x = upstream, large x = downstream
# The simulation convention:         small x = downstream (x=-0.21), large x = upstream (x=0.7)
# We evaluate the analytic profile at -x (negated) so the file is in simulation coordinates.
...
with open(outfile, "w") as f:
    ...
    f.write(f"# Shock at x = 0; simulation coordinates (downstream left, upstream right)\n")
...
# Sanity check: left boundary should be downstream, right boundary upstream
print(f"  Left  (x={x_sim[0]:.3f}, downstream):  rho={prof['rho'][0]:.4f}  "
      f"T_mat={T_mat_K[0]:.1f} K  vx={prof['vx'][0]:.2e}")
print(f"  Right (x={x_sim[-1]:.3f}, upstream):   rho={prof['rho'][-1]:.4f}  "
