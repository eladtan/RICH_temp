...
#SBATCH --output=/home/maorm/RICH/runs/RMTV/verification/lbphys_%j.out
#SBATCH --error=/home/maorm/RICH/runs/RMTV/verification/lbphys_%j.err
...
rmtv_output="$rmtv_dir/verification/lbphys_${SLURM_JOB_ID}"
cd "$rmtv_dir"
...
"$rmtv_mpirun" -np "$SLURM_NTASKS" --bind-to core "$rmtv_bin" \
    ...
    --photons 16 --initial-photons 16 --max-photons 32 --dump 100000 --max-steps 30 --ddmc-min-tau 3 --cost-model physics --cost-step-scale 1e-8 --cost-particle-scale 1 \
    --output "$rmtv_output"
