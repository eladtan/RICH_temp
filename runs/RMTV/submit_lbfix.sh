...
#SBATCH --output=/home/maorm/RICH/runs/RMTV/verification/lbfix_%j.out
#SBATCH --error=/home/maorm/RICH/runs/RMTV/verification/lbfix_%j.err
...
rmtv_output="$rmtv_dir/verification/lbfix_${SLURM_JOB_ID}"
cd "$rmtv_dir"
...
"$rmtv_mpirun" -np "$SLURM_NTASKS" --bind-to core "$rmtv_bin" \
    ...
    --photons 16 --initial-photons 16 --max-photons 32 --dump 100000 --max-steps 30 --ddmc-min-tau 3 --cost-step-scale 1 --cost-particle-scale 5 \
    --output "$rmtv_output"
