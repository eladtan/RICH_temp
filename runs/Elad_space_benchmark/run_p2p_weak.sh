...
#SBATCH --job-name="Space3D-WeakScale"
...
ln -s space_${SLURM_JOB_ID}_run.out space_WS_P2P_${SLURM_JOB_ID}_n${SLURM_NTASKS}.out
ln -s space_${SLURM_JOB_ID}_run.err space_WS_P2P_${SLURM_JOB_ID}_n${SLURM_NTASKS}.err
...
nodes = float("${NODES}")
dt_ref = float("${DT_REF}")
print("{:.12g}".format(dt_ref * (dt_ref_nodes / nodes) ** (1.0 / 3.0)))
PY
)}
...
ARGS="${NBASE} ${N_EMIT} ${CYCLES} --dt ${DT} --p2p" # --profiling-dir prof_strong_${NODES}"
EXEC="./rich"
