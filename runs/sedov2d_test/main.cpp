...
int main(void) {
    ...
    for (size_t i = 0; i < std::size_t(tess.GetPointNo()); ++i) {
        ...
        for (size_t j = 0; j < edge_index.size(); ++j) temp_vector.push_back(all_edges[edge_index[j]]);
    ...
    hdsim sim(cpu_tess, tess, outer_boundary, geometry, cells, eos, point_motion, interface_velocity_calc, source,
              time_step, flux_calc, extensive_update, cell_update,
              std::pair<std::vector<std::string>, std::vector<std::string> >(), &cpu_move);
#else
    hdsim sim(tess, outer_boundary, geometry, cells, eos, point_motion, interface_velocity_calc, source, time_step,
              flux_calc, extensive_update, cell_update);
#endif
    // Main loop
        ...
        try {
            ...
            if (sim.getCycle() % 20 == 0) {
                write_snapshot_to_hdf5(
                    sim, params.name + std::to_string(sim.getCycle()) + "_rank_" + std::to_string(rank) + ".h5", {},
                    false, true);
            }
...
#ifdef RICH_MPI
    write_snapshot_to_hdf5(
        sim, params.name + "_" + std::to_string(sim.getCycle()) + "_rank_" + std::to_string(rank) + ".h5");
    MPI_Finalize();
#else
