	...
	public:
		...
		std::pair<vector<size_t>, vector<Vector3D>> ToRefine(Tessellation3D const &tess, vector<ComputationalCell3D> const &cells, double time) const
		{
			...
			for (size_t i = 0; i < Norg; ++i)
			{
				...
				bool stream_cell = false;
				if(r_dist > 0.75 * Rt && r_dist < 0.5 * apocenter && cells[i].density > rho_x * 0.05)
					stream_cell = true;// stream_change
				if (tess.GetWidth(i) < min_cell_size * (r_dist < (0.65 * Rt) ? smooth_factor / 0.6 : 1))
				{
					...
					{
						if(tess.GetWidth(i) < 0.2 * min_cell_size)
							continue;// stream_change
					}
				}
				...
				if ((r_dist < (1.5 * Rt) || r_dist > 3 * apocenter) && (not first_refine) && (not stream_cell))
					continue;// stream_change

				double MaxMass2 = (tess.GetMeshPoint(i).x > (-apocenter)) ? MaxMass : MaxMass * 30;
				...
				if(r_dist < 0.5 * apocenter && cells[i].density > rho_x * 0.05)
				{// stream_change
					if(V > 0.01 * target_volume * std::pow(r_dist / Rt, 1.5))
					{
						res.push_back(i);
						continue;
					}
	...
	public:
		...
		std::pair<vector<size_t>, vector<double>> ToRemove(Tessellation3D const &tess, vector<ComputationalCell3D> const &cells, double time) const
		{
			...
			for (size_t i = 0; i < Norg; ++i)
			{
				...
				{
					if(r_org < 0.5 * apocenter && r_org > 0.75 * Rt && cells[i].density > rho_x * 0.05)
					{
						if(w < 0.2 * min_cell_size) // stream_change
						{
							res.push_back(i);
				...
				if((r_i < 1.25 * apocenter && cells[i].density > rho_x * 0.01 && r_i > Rt)  || (r_i < 0.5 * apocenter && ((Vol > 0.01 * z_abs * z_abs * z_abs) || z_abs < 20)))
					if(Vol > 1.25 * target_volume * std::pow(r_i / Rt, 1.5))
					{
						continue;
					}
				if(r_i < 0.5 * apocenter && cells[i].density > rho_x * 0.05)
				{// stream_change
					if(Vol > 0.0125 * target_volume * std::pow(r_i / Rt, 1.5))
					{
						continue;
					}
				}
