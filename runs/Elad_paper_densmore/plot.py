...
if __name__ == "__main__":
    ...
    profile_mpi = "densmore2012_mc_profile.txt"
    ref_file = "data/densmore2012_fig4_mc.csv"

    if not os.path.exists(profile_mpi):
        print(f"  [densmore2012_mc] no profile found for either MPI or serial variant")
        sys.exit(1)
    ...
    fig.tight_layout()
    fig.savefig("densmore2012_mc.png", dpi=200)
    fig.savefig("densmore2012_mc.pdf")
    plt.show()
    plt.close(fig)
    print(f"Saved densmore2012_mc.png/pdf")
