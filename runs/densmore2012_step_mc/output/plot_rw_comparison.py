...
rw_final = 'densmore_step_mc_rw_final.txt'
norw_final = 'densmore_step_mc_norw_final.txt'
...
snap_times = sorted(set(
    [f for f in glob.glob('densmore_step_mc_rw_snap*.txt')]
))
norw_snaps = sorted(glob.glob('densmore_step_mc_norw_snap*.txt'))
...
for sf in snap_times:
    idx = os.path.basename(sf).replace('densmore_step_mc_rw_snap', '').replace('.txt', '')
...
for sf in norw_snaps:
    idx = os.path.basename(sf).replace('densmore_step_mc_norw_snap', '').replace('.txt', '')
    x_s, T_s, _, _ = load_profile(sf)
    ax.plot(x_s, T_s / kev_kelvin, alpha=0.4, linewidth=0.8, label=f'snap {idx}')
ax.plot(x_norw, T_norw / kev_kelvin, 'r-', linewidth=2, label='no-RW final')
ax.plot(x_rw, T_rw / kev_kelvin, 'b--', linewidth=2, label='RW final')
...
fig.suptitle('Densmore 2012 Step — RW vs no-RW comparison\n'
