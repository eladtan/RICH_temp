...
/*
 * Densmore 2012 step-opacity test — Monte Carlo (multigroup IMC) version.
 *
 * Same problem as runs/densmore2012_step (multigroup diffusion):
 *   Domain:      x in [0, 3]
...
namespace
{
    class DensmoreMCOpacity : public OpacityCalculator
    {
    public:
        DensmoreMCOpacity(double sigma0_left, double sigma0_right,
...
int main(int argc, char *argv[])
{
  ...
  try
  {
    ArgumentParser arguments("Densmore 2012 step-opacity MC benchmark");
    arguments.addPositional<size_t>("Nx", "number of cells along x").required();
    arguments.addPositional<std::string>("prefix", "densmore_step_mc", "output prefix");
    ...
    auto opacityPtr = std::make_shared<DensmoreMCOpacity>(
    ...
    if(rank == 0)
    {
        std::cout << "Densmore 2012 step-opacity (MC multigroup IMC)"
                  << "\n  Nx=" << Nx
                  << ", G=" << G
    ...
    auto writeResults = [&](const std::string &filename, double time)
    {
        ...
        if(rank == 0)
        {
            ...
            out << "# Densmore2012 step MC multigroup  t=" << time << "  Nx=" << Nx << "\n";
