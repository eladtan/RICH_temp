...
int main(void)
{
	...
	std::stringstream ss;
	ss<<"R"<<R<<"M"<<M<<"BH"<<Mbh<<"beta"<<beta<<"S"<<static_cast<size_t>(smooth_factor*100)<<"n"<<n;
#ifdef hi_res
	ss<<"HiRes";
#endif
	if(rank == 0)
	...
	MultigroupDiffusion matrix_builder(opacity.energy_groups_center, opacity.energy_groups_boundary, opacity, D_boundary, eos, std::vector<std::string>(), flux_limit, hydro_on, compton_on, doppler_on, mixed_frame_on, 2000, protections_on);
	...
	while (sim->getTime() < tf)
	{
		...
		try
		{
			...
			new_dt = std::min(new_dt, 0.015);
			if(new_dt < 1e-4 && old_dt > 1e-4)
				new_dt = 1.01e-4;
