...
def measurement_label(args):
    if args.sum is None:
        return "Total benchmark step wall time (s)"
    if args.sum == 0:
...
def print_table(title, runs, tasks_per_node):
    if not runs:
        return
...
def main():
    ...
    if args.select == "all":
        print_table("RDMA parsed runs:", rdma_selected, args.tasks_per_node)
        if p2p_selected:
            print_table("P2P parsed runs:", p2p_selected, args.tasks_per_node)
        print_table("RDMA median curve:", rdma_line, args.tasks_per_node)
        if p2p_line:
