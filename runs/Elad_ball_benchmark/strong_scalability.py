...
def print_table(title, runs):
    if not runs:
        return

    print(title)
    columns = ("Processors", "Job", "Time(s)", "Measure", "Manager", "Steps", "dt", "Emitted/cyc", "File")
    rows = []
...
def main():
    ...
    if args.select == "all":
        print_table("RDMA parsed runs:", rdma_selected)
        if p2p_selected:
            print_table("P2P parsed runs:", p2p_selected)
        print_table("RDMA median curve:", rdma_line)
        if p2p_line:
