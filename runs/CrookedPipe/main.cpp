...
std::vector<Vector3D> GeneratePoints(const Vector3D &ll, const Vector3D &ur)
{
    std::vector<Vector3D> points;
    double dx = 0.01;
    double gridFactor = 1;
    size_t Np = 1e5 /*1e5*/ * gridFactor;
...
int main(int argc, char *argv[])
{
    ...
    std::shared_ptr<RadiationIMC> physics = std::make_shared<RadiationIMC>(tess, boundaryCond, cells, conserved, eos, opacity, 100);
