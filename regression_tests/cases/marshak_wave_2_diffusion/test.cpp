...
// Problem 2: Krief & McClarren (2024) Test 3
// kappa_R = kappa_P = 100*(T/keV)^{-3} (equilibrium limit)
...
int main(void)
{
	...
	// kappa_R = kappa_P = 100*(keV_K/T)^3
	// D = c/(3*100*keV_K^3*T^{-3}) = c*T^3/(300*keV_K^3)
	// planck = 100*keV_K^3*T^{-3}
	double const D0 = CG::speed_of_light / (300.0 * std::pow(keV_K, 3.0));
	double const planck0 = 100.0 * std::pow(keV_K, 3.0);
	...
	double const T_bath_init = 1.014565 * std::pow(1e-15 / 1e-9, 1.0 / 3.0) * keV_K;
