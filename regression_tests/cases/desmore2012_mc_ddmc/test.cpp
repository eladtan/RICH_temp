...
/*
 * Densmore 2012 heterogeneous step-opacity — MPI Monte Carlo regression test (DDMC).
 *
 * Same setup as desmore2012_mc, but with DDMC enabled instead of plain MC transport.
 * Random walk remains disabled.
 *
 * Outputs desmore2012_mc_ddmc_profile.txt (x, T_K) for comparison with the
...
int main(int argc, char *argv[])
{
  ...
  try
  {
    ...
    if(rank == 0)
    {
        std::cout << "Densmore 2012 heterogeneous step-opacity (MC regression, DDMC)"
                  << "\n  Nx=" << Nx << ", G=" << G
    ...
    if(rank == 0)
    {
        ...
        std::string const profilePath = caseDir + "/desmore2012_mc_ddmc_profile.txt";
        std::ofstream out(profilePath);
        out << "# Densmore2012 MC regression (DDMC)  t=" << simTime << "  Nx=" << Nx << "\n";
        out << "# x(cm)  T(K)\n";
