...
int main(void)
{
    ...
    while (simulation.GetTime() < T_END) {
        try {
            if (simulation.GetCycle() % LOG_INTERVAL == 0) {
                double local_ekz = 0.0;
                size_t const Nloc = sim.getTessellation().GetPointNo();
                for (size_t i = 0; i < Nloc; ++i) {
                    double vz = sim.getCells()[i].velocity.z;
                    double rho = sim.getCells()[i].density;
                    double vol = sim.getTessellation().GetVolume(i);
...
#endif
                if (rank == 0) {
                    ek_file << simulation.GetTime() << " " << global_ekz << "\n";
                    ek_file.flush();
                }
            }
    ...
    size_t const Nfinal = sim.getTessellation().GetPointNo();
