...
// Problem 1: Krief & McClarren (2024) Test 2
...
int main(void)
{
	...
	// EOS: e = f * T^beta * rho^{-mu}, u = rho*e = f*T^beta*rho^{1-mu}
	// u(T) = 6.860085e14 * (T/keV_K)^4 => f = 6.860085e14/keV_K^4, beta=4, mu=0
	double const f_eos = 6.860085e14 / std::pow(keV_K, 4.0);
	IdealGas eos(1.4, f_eos, 4.0, 0.0);

	// Opacity: kappa_R = 100*(keV_K/T)^3
	// D = c/(3*kappa_R) = c*T^3/(300*keV_K^3)
	// kappa_P = 0.001*kappa_R = 0.1*keV_K^3*T^{-3}
	double const D0 = CG::speed_of_light / (300.0 * std::pow(keV_K, 3.0));
	...
	double const T_bath_init = 1.008038 * std::pow(1e-15 / 1e-9, 1.0 / 3.0) * keV_K;
