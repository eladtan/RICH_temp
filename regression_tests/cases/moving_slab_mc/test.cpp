...
static const OpacityRow OPACITY_TABLE[124] = {
...
class MovingSlabOpacity : public OpacityCalculator
{
public:
    MovingSlabOpacity(double rho_slab,
        ...
        : rho_slab_(rho_slab)
    {
        ...
        for (size_t g = 0; g < N_OPACITY_GROUPS; ++g)
            sigma_g_[g] = rho_slab_ * OPACITY_TABLE[g].kappa;
    }
    ...
    double CalcPlanckOpacity(const ComputationalCell3D &cell) const override
    {
        ...
        for (size_t g = 0; g < N_OPACITY_GROUPS; ++g)
        {
            double Elo = OPACITY_TABLE[g].nu_min * units::kev;
            double Ehi = OPACITY_TABLE[g].nu_max * units::kev;
...
private:
    double rho_slab_;
    double sigma_g_[N_OPACITY_GROUPS];
};
...
public:
    ...
    MonteCarloParticleStatus apply(MonteCarloParticle<Vector3D, Tessellation3D> &particle) override
    {
        ...
        for (const auto &face : faces)
        {
            ...
            double absU = std::sqrt(abs(v) * abs(u));
        ...
        std::cerr << "MovingSlabBC: particle not on any boundary" << std::endl;
        exit(1);
    }
...
int main(int argc, char *argv[])
{
    ...
    try
    {
        ...
        static_assert(G == 124, "Moving slab benchmark requires exactly 124 energy groups");
        ...
        // --- 124-group energy grid from the opacity table (in CGS: erg) ---
        std::vector<double> energy_groups_center(G);
        std::vector<double> energy_groups_boundary(G + 1);
        for (size_t g = 0; g < G; ++g)
        {
            energy_groups_boundary[g]     = OPACITY_TABLE[g].nu_min * units::kev;
            energy_groups_boundary[g + 1] = OPACITY_TABLE[g].nu_max * units::kev;
        ...
        // tess.SetKernel(std::make_shared<Kernelization3D::Rectangle>(ll, ur));
        ...
        auto opacityPtr = std::make_shared<MovingSlabOpacity>(
            rho_slab, energy_groups_center, energy_groups_boundary);
        ...
        auto remeshGenerator = [&, prev_t = 0.0](const Tessellation3D &t, double /*time*/) mutable -> std::vector<Vector3D>
        {
            double t_now = sim.GetTime();
            double new_ll_x = v_slab * t_now;
            Vector3D new_ll = Vector3D(new_ll_x, -cellHalfYZ, -cellHalfYZ);
            // tess.SetBox(new_ll, ur, std::make_shared<Kernelization3D::Rectangle>(new_ll, ur));
            tess.SetBox(new_ll, ur);
            
            double slab_front_old = L_slab + v_slab * prev_t;
            double slab_front_new = L_slab + v_slab * t_now;
            double dx = v_slab * (t_now - prev_t);
            prev_t = t_now;

            size_t localN = t.GetPointNo();
            std::vector<Vector3D> localPoints(localN);
            for (size_t i = 0; i < localN; ++i)
            {
                Vector3D p = t.GetMeshPoint(i);
        ...
        auto remeshStep = std::make_shared<RemeshStep>(tess, cells, extensives, remeshGenerator);

        remeshStep->setPostRebuild([&]()
        {
            auto &parts = mcStep->getParticles();
            auto boxCoords = tess.GetBoxCoordinates();
            const Vector3D &ll_box = boxCoords.first;
            const Vector3D &ur_box = boxCoords.second;
            parts.erase(
                std::remove_if(parts.begin(), parts.end(),
                    [&ll_box, &ur_box](const auto &p) {
                        return p.location.x < ll_box.x || p.location.x > ur_box.x;
                    }),
                parts.end());

            size_t N_local = tess.GetPointNo();
            extensives.resize(N_local);
        ...
        if (rank == 0)
        {
            std::cout << "Moving slab MC benchmark (original vacuum): "
                      << G << " groups, "
        ...
        // Collect and average EgTA across all YZ cells at the observer x
        ...
        if (rank == writerRank)
        {
            ...
            out << "# Moving slab MC benchmark (original_vacuum)\n";
            ...
            {
                out << g
                    << " " << OPACITY_TABLE[g].nu_min
                    << " " << OPACITY_TABLE[g].nu_max
