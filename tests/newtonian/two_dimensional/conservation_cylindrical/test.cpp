...
namespace {
    ...
    bool same_side(const Vector2D& p1, const Vector2D& p2, const Vector2D& a, const Vector2D& b) {
    ...
    class Triangle : public Shape2D {
       public:
        ...
        Triangle(const Vector2D& p1_i, const Vector2D& p2_i, const Vector2D& p3_i) : p1(p1_i), p2(p2_i), p3(p3_i) {}

        bool operator()(const Vector2D& p) const {
            return same_side(p, p1, p2, p3) && same_side(p, p2, p1, p3) && same_side(p, p3, p1, p2);
        }
    };
    ...
    vector<ComputationalCell> calc_init_cond(const Tessellation& tess) {
        ...
        for (size_t i = 0; i < res.size(); ++i) {
            const Triangle triangle(Vector2D(0.5, 0.6), Vector2D(0.7, 0.5), Vector2D(0.4, 0.4));
        ...
        SimData(void)
            : pg_(Vector2D(0, 0), Vector2D(0, 1)),
...
#else
              init_points_(cartesian_mesh(30, 30, outer_.getBoundary().first, outer_.getBoundary().second)),
              tess_(init_points_, outer_),
#endif

              triangle_(Vector2D(0.5, 0.6), Vector2D(0.7, 0.5), Vector2D(0.4, 0.4)),
              eos_(5. / 3.),
              pm_(),
              evc_(),
              rs_(),
              ...
              sim_(
                  ...
                  proc_tess_,
#endif
                  tess_, outer_, pg_, calc_init_cond(tess_), eos_, pm_, evc_, force_, tsf_, fc_, eu_, cu_,
                  {vector<string>(1, "tracer"), vector<string>()}) {
        }

        hdsim& getSim(void) { return sim_; }
    ...
    class WriteConserved : public DiagnosticFunction {
       public:
        ...
        void operator()(hdsim const& sim) { cons_.push_back(total_conserved(sim)); }
        ...
        ~WriteConserved(void) {
            ...
            if (rank_ == 0) {
                ...
                for (size_t i = 0; i < cons_.size(); ++i)
                    f << cons_[i].mass << " " << cons_[i].momentum.x << " " << cons_[i].momentum.y << " "
                      << cons_[i].energy << " " << cons_[i].tracers[0] << "\n";
                f.close();
#ifdef RICH_MPI
            }
#endif
...
namespace {
    void my_main_loop(hdsim& sim) {
        ...
        main_loop(sim, term_cond, &hdsim::TimeAdvance, &diag);
    }
}  // namespace
