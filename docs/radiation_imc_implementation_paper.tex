...
Third, if \texttt{diffusionPressureGradient} is active, the code computes a
one-dimensional estimate of \(\partial_x\langle E_{\rm r}\rangle_t\) and
applies
\begin{equation}
  \Delta p_{x,i}
  =
  -\dt\,V_i\,\frac{1}{3}\,
  \partial_x\langle E_{\rm r}\rangle_{t,i}.
  \label{eq:pressure-gradient}
\end{equation}
The implementation searches for left and right neighbors by comparing neighbor
directions with the \(x\)-axis. At physical boundaries it falls back to a
one-sided difference. The source comments mark this pressure-gradient branch as
currently one-dimensional, even though the rest of the transport is fully
three-dimensional.
