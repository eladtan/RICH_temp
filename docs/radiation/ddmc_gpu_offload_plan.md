...
| MPI/handoff | `MonteCarloManager` keeps device survivors resident but compacts particles that leave the local grid and passes remote particles to the selected communication engine. | A DDMC leak to a ghost cell must become a terminal `CELL_MOVE`, carry the DDMC flags and `pendingFlux`, and follow this same path. |
