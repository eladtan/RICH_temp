...
- **Timestep-based removal**: Cells too small for stable timestep `dt < 0.02 * time_Rt` are removed
- **Mass threshold**: Cells below `3.5e-8 * Mstar * min(1, (t/t_apo)^2)` mass (time-dependent)
...
See [Example: TDE Simulation](../examples/tde-simulation.md) for the full walkthrough.
